...
class RecommendDownload(BaseDownload):
    def __init__(self):
        BaseDownload.__init__(self)
        Log.log_info.info('推荐模式 下载 线程开启...')
        ...
        try:
            while True:
                if not self.m_switch:
                    Log.log_info.info('推荐模式 下载 线程正在结束...')
                    return
                time.sleep(1)
                self._download()
    ...
    def _download(self):
        try:
            download = GlobalEnv.get_recommend_queue_download()
            if download:
                ...
                if download['web_download']:
                    EventRuntime().on_event(EventType.FUNC_DOWNLOAD_APK, url=download['web_download'], 
                                            dirname=GlobalEnv.global_file_path + '/recommend/' + download['web_title'], 
